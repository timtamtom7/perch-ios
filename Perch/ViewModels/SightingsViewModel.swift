...
class SightingsViewModel: ObservableObject {
    ...
    var recentSightings: [Sighting] {
        Array(sightings.sorted { $0.date > $1.date }.prefix(5))
    }
    ...
    func loadSightings() {
        ...
        if let savedData = UserDefaults.standard.data(forKey: "perchSightings"),
           let savedSightings = try? JSONDecoder().decode([Sighting].self, from: savedData) {
    ...
    func saveSightings() {
        if let encoded = try? JSONEncoder().encode(sightings) {
            UserDefaults.standard.set(encoded, forKey: "perchSightings")
        }
    }
